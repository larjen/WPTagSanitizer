...
class WPTagSanitizerAdmin extends WPTagSanitizer {
    ...
    static function plugin_options() {
        ...
        if (isset($_POST[self::$plugin_name."_JSONTABLE"])) {
            $textAreaValue = stripslashes($_POST[self::$plugin_name."_JSONTABLE"]);
            $transformTable = self::getTransformTable($textAreaValue);

            if ($transformTable != false) {
                update_option(self::$plugin_name."_JSONTABLE", $textAreaValue);
                update_option(self::$plugin_name."_TRANSFORMTABLE", $transformTable);
            }
        } else {
            $textAreaValue = get_option(self::$plugin_name."_JSONTABLE");
        }
        ...
        if (self::$debug) {
            ...
            echo 'get_option(self::$plugin_name."_JSONTABLE")=' . print_r(get_option(self::$plugin_name."_JSONTABLE")) . PHP_EOL;
            echo 'get_option(self::$plugin_name."_TRANSFORMTABLE")=' . print_r(get_option(self::$plugin_name."_TRANSFORMTABLE")) . PHP_EOL;
            echo '</pre>';
        }
